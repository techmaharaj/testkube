		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			run := func(index int64, spec *testworkflowsv1.TestWorkflowSpec) bool {
				...
				machine := expressionstcl.NewMachine().
					...
					Register("resourceSuffix", fmt.Sprintf("%d", index)).
				...
				for _, item := range bundle.Secrets {
					...
					if err != nil {
						fmt.Printf("worker %d: failed to deploy secrets: %s\n", index+1, err.Error())
						return false
					}
				}
				...
				if err != nil {
					fmt.Printf("worker %d: error: failed to deploy job: %s\n", index+1, err.Error())
					return false
				}
					...
					if total != 0 && total == paused {
						...
						for index := range statuses {
							go func(index int64) {
								...
								if err != nil {
									fmt.Printf("worker %d: warning: failed to resume: %s\n", index+1, err.Error())
								}
							}(index)
						}
						wg.Wait()
